...
// RandomFactionHandler allows users to name and select a homeland
func RandomFactionHandler(w http.ResponseWriter, req *http.Request) {
	...
	if req.Method == "GET" {
		...
		// Render page
		Render(w, "templates/random_faction.html", wc)
	...
	if req.Method == "POST" {
		...
		numChars, err := strconv.Atoi(number)
		if err != nil {
			numChars = 1
		}
		...
		for i := 1; i <= numChars; i++ {
			...
			fac.CharacterModelSlugs = append(fac.CharacterModelSlugs, cm.Slug)
